...
export default function Sidebar() {
  ...
  return (
    <aside className="fixed left-0 top-0 z-40 flex h-screen w-64 flex-col border-r border-slate-200/60 bg-white">
      ...
      {/* User Dropdown */}
      <div className="relative border-t border-slate-200/60 p-4">
        <div
          onClick={() => setUserMenuOpen(!userMenuOpen)}
          className="flex items-center gap-3 rounded-xl p-2 transition hover:bg-slate-50 cursor-pointer"
        >
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-slate-500 truncate">usuario@gmail.com</p>
          </div>
          <svg className={`h-4 w-4 text-slate-400 transition-transform ${userMenuOpen ? "rotate-180" : ""}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
          </svg>
        </div>

        {userMenuOpen && (
          <div className="absolute bottom-full left-4 right-4 mb-2 rounded-xl border border-slate-200 bg-white py-1 shadow-lg">
            <Link
              href="/profile"
